...
% - matrix: matrix containing EMG envelopes to be analyzed
%
% OUTPUT
% - synergy: structure containing the muscle synergies extracted for each
%            tested number of synergies (n_input)
% ---------------------------------------------------------
...
for ii = opts.n_input 
    ...
    % Any potential negative value in data matrix will be set to zero
    % V = V.*(V>0);
    matrix(matrix<0) = 0;
    ...
    % extract only the number of synergies equal to the number of muscles
    ...
    % Factorize input matrix by means of the NNMF algorithm
